...
def test_package_can_be_imported() -> None:
    package = import_module("template_fastapi_react")
...
def test_template_project_metadata_describes_scaffold() -> None:
    ...
    assert project["name"] == "template-fastapi-react"
    assert (
        project["description"]
...
def test_template_declares_typed_src_package() -> None:
    ...
    assert (ROOT / "src" / "template_fastapi_react" / "py.typed").is_file()
    assert "Typing :: Typed" in project["classifiers"]
    assert hatch_targets["wheel"]["packages"] == [
        "src/template_fastapi_react",
    ]
    assert tool["coverage"]["run"]["source"] == ["template_fastapi_react"]
